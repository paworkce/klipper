...
class DumpTrapQ:
    ...
    def log_trapq(self, data):
        ...
        for i, m in enumerate(data):
            out.append("move %d: pt=%.6f mt=%.6f sv=%.6f a=%.6f"
                       ...
                       " sp=(%.6f,%.6f,%.6f,%.6f,%.6f) ar=(%.6f,%.6f,%.6f,%.6f,%.6f)"
                       % (i, m.print_time, m.move_t, m.start_v, m.accel,
                          #m.start_x, m.start_y, m.start_z, m.x_r, m.y_r, m.z_r))
                           m.start_x, m.start_y, m.start_z, m.start_r, m.start_t,
                            m.x_r, m.y_r, m.z_r, m.r_r, m.t_r))
        logging.info('\n'.join(out))
    def get_trapq_position(self, print_time):
        ...
        pos = (move.start_x + move.x_r * dist, move.start_y + move.y_r * dist,
               ...
               move.start_z + move.z_r * dist,
               move.start_r + move.r_r * dist, move.start_t + move.t_r * dist)
    ...
    def _process_batch(self, eventtime):
        ...
        d = [(m.print_time, m.move_t, m.start_v, m.accel,
              #(m.start_x, m.start_y, m.start_z), (m.x_r, m.y_r, m.z_r))
              (m.start_x, m.start_y, m.start_z, m.start_r, m.start_t), (m.x_r, m.y_r, m.z_r, m.r_r, m.t_r))
             for m in data]
