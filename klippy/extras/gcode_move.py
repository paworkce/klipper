...
class GCodeMove:
    ...
    def cmd_G92(self, gcmd):
        ...
        offsets = [ gcmd.get_float(a, None) for a in 'XYZRTE' ]
    ...
    def cmd_M114(self, gcmd):
        ...
        gcmd.respond_raw("X:%.3f Y:%.3f Z:%.3f R:%.3f T:%.3f E:%.3f" % tuple(p))
    ...
    def cmd_SET_GCODE_OFFSET(self, gcmd):
        ...
        for pos, axis in enumerate('XYZRTE'):
    ...
    def cmd_GET_POSITION(self, gcmd):
        ...
        kinfo = zip("XYZRT", kin.calc_position(dict(cinfo)))
        kin_pos = " ".join(["%s:%.6f" % (a, v) for a, v in kinfo])
        toolhead_pos = " ".join(["%s:%.6f" % (a, v) for a, v in zip(
            #"XYZE", toolhead.get_position())])
            "XYZRTE", toolhead.get_position())])
        gcode_pos = " ".join(["%s:%.6f"  % (a, v)
                              #for a, v in zip("XYZE", self.last_position)])
                              for a, v in zip("XYZRTE", self.last_position)])
        base_pos = " ".join(["%s:%.6f"  % (a, v)
                             #for a, v in zip("XYZE", self.base_position)])
                              for a, v in zip("XYZRTE", self.base_position)])
        homing_pos = " ".join(["%s:%.6f"  % (a, v)
                               #for a, v in zip("XYZ", self.homing_position)])
                               for a, v in zip("XYZRTE", self.homing_position)])
